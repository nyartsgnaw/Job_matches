...
#    model = create_LSTM(input_dim=INPUT_DIM,output_dim=OUTPUT_DIM,embedding_matrix=embedding_matrix)
    ...
    model = load_model(os.path.join(CWDIR,'./../models/new3.model'))
    from keras.optimizers import SGD, Adam, RMSprop
    adam=Adam(lr=0.005, beta_1=0.9 ,decay=0.001)

    model.compile(loss='cosine_proximity', optimizer=adam, metrics=['mse'])

    model = train_model(model,X_train=X_train.reshape([-1,INPUT_DIM,TIME_STEPS,1]),Y_train=Y_train,verbose=1,n_epoch=400,validation_split=0,patience=100,model_path='LSTM.model',log_path='LSTM_logs3.csv')
    model.save(os.path.join(CWDIR,'./../models/new3.model'))
    ...
    df.to_csv(os.path.join(CWDIR,'./../results3.csv'),index=False)
