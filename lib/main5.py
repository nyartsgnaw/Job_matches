...
if __name__ == '__main__':
    ...
    # create the model    
    import_local_package(os.path.join(CWDIR,'./models/LSTM_5.py'),[])
    embedding_matrix = []
...
#    model = load_model(os.path.join(CWDIR,'./../models/new.model'))
    ...
    model = train_model(model,X_train=X_train.reshape([-1,INPUT_DIM,TIME_STEPS,1]),Y_train=Y_train,verbose=1,n_epoch=200,validation_split=0,patience=100,model_path='LSTM5.model',log_path='LSTM_logs5.csv')
    ...
    df.to_csv(os.path.join(CWDIR,'./../results5.csv'),index=False)
    ...
    model.save(os.path.join(CWDIR,'./../models/new5.model'))
