...
if __name__ == '__main__':
    ...
    path_JT = './../models/job_title_fasttext.bin'
    if os.path.isfile(path_JT):
        model_JT = fasttext.load_model(path_JT, encoding='utf-8')
    else:
        model_JT = train_fasttext(titles,path_JT,vector_dim=OUTPUT_DIM)
    
    labels = np.array([model_JT[x] for x in titles])
    ...
    if os.path.isfile(path_JD):
        model_JD = fasttext.load_model(path_JD, encoding='utf-8')
    else:
        model_JD = train_fasttext(texts,path_JD,vector_dim=OUTPUT_DIM)
